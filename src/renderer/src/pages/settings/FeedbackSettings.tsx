...
import { submitFeedback } from '@renderer/services/AdminService/Feedback'
...
const FeedbackForm: FC = () => {
  ...
  if (isSubmitted) {
    return (
      <SettingContainer>
        <SettingGroup>
          <SettingTitle>{t('settings.feedback.title')}</SettingTitle>
          <SettingDivider />
          <Result
            status="success"
        ...
        <Form form={form} layout="vertical" onFinish={onFinish}>
          ...
          <Form.Item name="type" label="类型" rules={[{ required: true, message: '请选择类型' }]}>
            <Select placeholder="请选择反馈类型">
              <Option value="suggestion">{t('settings.feedback.type.suggestion')}</Option>
              <Option value="question">{t('settings.feedback.type.question')}</Option>
            </Select>
          </Form.Item>
