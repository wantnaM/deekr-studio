...
// 新的默认小程序配置（来自 src 目录，已去除 subject 字段）
const NEW_ORIGIN_DEFAULT_MIN_APPS: MinAppType[] = [
  // {
  //   id: 'openai',
  //   name: 'ChatGPT',
  //   url: 'https://chatgpt.com/',
...
let allMinApps = [...NEW_ORIGIN_DEFAULT_MIN_APPS, ...(await loadCustomMiniApp())]

function updateAllMinApps(apps: MinAppType[]) {
  allMinApps = apps
}

export { allMinApps, loadCustomMiniApp, NEW_ORIGIN_DEFAULT_MIN_APPS as ORIGIN_DEFAULT_MIN_APPS, updateAllMinApps }
